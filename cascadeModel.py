...
For each rainfall volume R above a certain threshold
the weights are calculated.

This MDRC is a Microcanonical MDRC, which means that
in each disagregation level the rainfall volume is preserved,
so that R1.W1 + R2.W2 = R --> W1 + W2 = 1.
...
min_nbr_vals = 2.  # min nbr of weights for Prob cals
...
if np.isclose(df_5m_to_30m.sum(),
              df_5m_to_60m.sum(),
              in_df_5m.sum()).any() is True:
    print('error in resampling')

# save resampled df of 30min timestamp, used in model evaluation
df_5m_to_30m.to_csv(os.path.join(out_dir, 'resampled 30min.csv'), sep=df_sep)
...
# verify that volumes are conserved after resampling
if np.isclose(df_5m_to_15m.sum(),
              in_df_5m.sum()).any() is True:
    print('error in resampling')

# save df 15min, used in evaluation later
df_5m_to_15m.to_csv(os.path.join(out_dir,
                                 'resampled 15min.csv'),
                    sep=df_sep)
...
def probWeightsMonth(weight_dict_month, min_nbr_values, cascade_level):
    ...
    all_vals_month = {k: [] for k in months}

    # define dict to hold prob per month
    global prob_month_P1
    prob_month_P1 = {k: [] for k in months}
    prob_month_P0 = {k: [] for k in months}
    weights_month_P = {k: [] for k in months}

    # iterate for every month
    for month in weight_dict_month.keys():
        # go for every stn in dict month
        for stn in weight_dict_month[month].keys():
            # count vals per month
            ct = len(weight_dict_month[month][stn])
            # append to dict
            all_vals_month[month].append(ct)
            # go through stn W vals
            for v in weight_dict_month[month][stn]:
                # get P(W=1) per month
                if v == 1:
                    weights_month_P1[month].append(v)
                # get P(W=0) per month
                elif v == 0:
                    weights_month_P0[month].append(v)
                # get P != P01 per month
                elif 0 < v < 1:
                    weights_month_P[month].append(v)

        # frequency P01 per month
    for month in weight_dict_month.keys():
        if (len(weights_month_P[month]) +
            len(weights_month_P0[month]) +
            len(weights_month_P1[month])) !=\
                np.sum(all_vals_month[month]):
                    print('no errors')
                    break
#        print(month, len(weights_month_P0[month]),
#              len(weights_month_P1[month]),
#              len(weights_month_P[month]))

        if len((weights_month_P1[month])) >= min_nbr_values:
            prob_month_P1[month].append(
                    len(weights_month_P1[month]) / np.sum(
                            all_vals_month[month]))

        if len((weights_month_P0[month])) >= min_nbr_values:
            prob_month_P0[month].append(
                    len(weights_month_P0[month]) / np.sum(
                            all_vals_month[month]))
...
print('done with fitting beta to weights, finding unbounded model')
...
def volumeDependacy(in_df_w_stn_dict, cascade_level):
    ...
    for stn in in_df_w_stn_dict.keys():
        ...
        # what to minimise, cal fct on vals and optmise to find params of fct
...
def assignWvals(df_rain_upper_level,
    ...
    df_rain_upper_level = df_rain_upper_level[df_rain_upper_level >
                                              R_threshhold]
...
# =============================================================================
            ...
            # store simulated vals in df_out
            df_output_L1.loc[idx1_1,
                             'unbounded rainfall %s' % cascade_level_1] = R1_u

            df_output_L1.loc[idx2_2,
                             'unbounded rainfall %s' % cascade_level_1] = R2_u
...
# =============================================================================
            ...
            df_output_L2.loc[idx_l2_1,
                             'baseline rainfall %s' % cascade_level_2] = R1_2_1

            df_output_L2.loc[idx_l2_2,
                             'baseline rainfall %s' % cascade_level_2] = R2_2_1
            ...
            df_output_L2.loc[idx_l2_3,
                             'baseline rainfall %s' % cascade_level_2] = R1_2_2

            df_output_L2.loc[idx_l2_4,
                             'baseline rainfall %s' % cascade_level_2] = R2_2_2
...
# =============================================================================
            ...
            df_output_L2.loc[idx_l2_2, 'unbounded rainfall %s'
...
while simulation_nbr <= 1000:  # upper limit to define nbr of simulations
    simulation_nbr += 1
...
def buildLorenzCurve(in_sim_dfs_L1_dict, orig_df,
                     cascade_level):
...
#                print(orig_rain_vals)
                ...
                for th_o in orig_rain_vals_unique:
                    nbr_o_vals_per_class.append((orig_rain_vals <= th_o).sum())
                ...
                df_o_rain_occurences.rename(columns={0: 'Ni O'}, inplace=True)
...
lorenz_sim_L1 = buildLorenzCurve(evaluation_L1, df_5m_to_30m,
                                 cascade_level_1)

lorenz_sim_L2 = buildLorenzCurve(evaluation_L1, df_5m_to_15m,
                                 cascade_level_2)
