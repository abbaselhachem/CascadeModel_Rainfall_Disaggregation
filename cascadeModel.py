...
First read rainfall data on a fine resolution (5min) and aggregate the data
...
in_df_5m.drop(in_df_5m.columns[[0, 3, 10, 13]], axis=1,
...
#==============================================================================
# Cascade Model (60min-->30min-->15min) (3 Levels)
#==============================================================================
'''
...
freq_level0 = '60T' #60min

#define time of sub intervals, level one
freq_level1 = '30T' #30min

#aggregate 5min df to 60min df, input data
df_5m_to_60m = in_df_5m.resample(freq_level0,
                                 label='right',
                                 closed='right').sum()

#aggeragete 5min data to 30min data using freq_level1
df_5m_to_30m = in_df_5m.resample(freq_level1,
                                 label='right',
                                 closed='right').sum()

#verify that volumes are conserved after resampling
assert np.isclose(df_5m_to_30m.sum(),
                  df_5m_to_60m.sum(),
                  in_df_5m.sum()).any() == True,\
                  'error in resampling'

#save resampled df of 30min timestamp, used in model evaluation
df_5m_to_30m.to_csv('resampled 30min.csv', sep=df_sep)

#define time delta to substract from df index to get values in other df
time_delta_30m_left = 3600. #s 1 hour
time_delta_30m_right = time_delta_30m_left/2 #1800s 30min
...
neglectedData(df_5m_to_60m, threshhold)
...
def cascadeWeightsLevel_1(df_level_1, df_level_2, R_threshhold):
    ...
    for station in df_level_1.columns:
        ...
        for idx in df_level_1[station].index:
            ...
            #end idx of sub int 1 (00:30:00)
            idx1_1 = idx - pd.\
                Timedelta(seconds=time_delta_30m_right)
...
cascadeWeightsLevel_1(df_5m_to_60m, df_5m_to_30m, threshhold)
...
4 sub interval if (0-30) then (0-15) (15-30)
               if (30-0) then (30-45) (45-60)
...
#define time of sub intervals (15min) level two
freq_level2 = '15T'

#define time delta to substract from df index
#to get values in other df
time_delta_15m_left = time_delta_30m_left/2 #s 30min
time_delta_15m_right = time_delta_15m_left/2 #s 15min

#aggeragete 5min data to 15min data using freq_level2
df_5m_to_15m = in_df_5m.resample(freq_level2,
                                 label='right',
                                 closed='right').sum()

#verify that volumes are conserved after resampling
assert np.isclose(df_5m_to_15m.sum(),
                  in_df_5m.sum()).any() == True,\
                  'error in resampling'

#save df 15min, used in evaluation later
df_5m_to_15m.to_csv('resampled 15min.csv', sep=df_sep)
    ...
    for station in df_level_2.columns:
        ...
        for idx in df_level_2[station].index:
            ...
            if idx.minute == 30:
                ...
                '''disaggregate to sub-int (0-15; 15-30)'''
                ...
                #end idx (00:15:00) (sum sub-interval left 1)
                idx2_1_0 = idx - pd.\
                    Timedelta(seconds=time_delta_15m_right)
                assert idx2_1_0.minute == 15, 'error locating 1st idx'
                ...
                #end idx (00:30:00), (sum sub-interval right 1)
                idx2_2_1 = idx
                assert idx2_2_1.minute == 30, 'error locating 2nd idx'
               ...
               #append weights to month_dict_right
                ...
                '''disaggregate to sub int (30-45; 45-60)'''
                ...
                idx2_2_1 = idx - pd.\
                    Timedelta(seconds=time_delta_15m_right)
                assert idx2_2_1.minute == 45. , 'error locating 3rd idx'
                ...
                #idx (00:00:00)
                idx2_2_2 = idx
                assert idx2_2_2.minute == 0.,'error locating 4th idx'
...
cascadeWeightslevel_2(df_5m_to_30m, df_5m_to_15m, threshhold)
...
def maxLikelihood(dict_weights, cascade_level):
    ...
    bounds = [(2, 5.), (2, 5.)] #change this to improve fit
...
print('done with fitting beta to weights, finding boxes next')

#==============================================================================
# find boxes (need fixing) (also add second level)
#==============================================================================

#Isolated(010), Preceded(110), Followed(011), Enclosed(111)
boxes = {'Isolated': {k:[] for k in in_df_5m.columns},
         'Preceded': {k:[] for k in in_df_5m.columns},
         'Followed': {k:[] for k in in_df_5m.columns},
         'Enclosed': {k:[] for k in in_df_5m.columns}}

#define epsilon as lower boundary, if value near eps then it is 0
time_delta_30m_left = 3600. #30min

def getBoxes(df_level_1, R_threshhold):

    '''
        Idea: Separated time intervals into four classes:
            starting (preceded by dry and followed by wet),
            ending (preceded by wet and followed by dry),
            enclosed (bounded by wet), and isolated (bounded by dry).

        In the model apply a distinct distribution function for W
        to each interval class. (improvements for later)

    '''

    global df_out_boxes
    df_out_boxes = pd.DataFrame(index=df_level_1.columns)

    #select values above threhhold
    df_level_1 = df_level_1[df_level_1 >= R_threshhold]
    ...
    for station in df_level_1.columns:
        ...
        #go through index df level 1, start from 1st date
        for idx in df_level_1[station].index:

            #get boxes, exclude first and last index
            if idx != df_level_1[station].index[0]\
                and idx != df_level_1[station].index[-1]:

                #get rainfall value upper df
                val = df_level_1[station].loc[idx]

                #value b4 minus 60min
                try:
                    idx0_0 = idx - pd.Timedelta(seconds=time_delta_30m_left)
                    val0_0 = df_level_1[station].loc[idx0_0]

                    #value after plus 60min
                    idx0_1 = idx + pd.Timedelta(seconds=time_delta_30m_left)
                    val0_1 = df_level_1[station].loc[idx0_1]

                    if val > 0.: #start wet

                        #check if b4 and after was wet or not
                        if val0_0 == 0 and val0_1 ==0:
                            boxes['Isolated'][station].append(1)

                        #check if b4 was wet and after was dry
                        elif val0_0 > 0 and val0_1 == 0.:
                            boxes['Preceded'][station].append(1)

                        #check if b4 was dry and after was wet
                        elif val0_0 == 0. and val0_1 > 0:
                            boxes['Followed'][station].append(1)

                        #check if b4 was wet and after was wet
                        elif val0_0 > 0 and  val0_1 > 0:
                            boxes['Enclosed'][station].append(1)
#                    elif val == 0: #start dry

                except Exception as msg:
                    print(msg)

            #save df output
            df_out_boxes.loc[station, 'Isolated'] =\
                                    sum(boxes['Isolated'][station])
            df_out_boxes.loc[station, 'Preceded'] =\
                                        sum(boxes['Preceded'][station])
            df_out_boxes.loc[station, 'Followed'] =\
                                        sum(boxes['Followed'][station])
            df_out_boxes.loc[station, 'Enclosed'] =\
                                        sum(boxes['Enclosed'][station])
            df_out_boxes.loc[station, 'All Events'] =\
                                        len(df_level_1[station].values)

    df_out_boxes.to_csv(os.path.join(out_dir, '60min df boxes.csv'),
                        sep=df_sep)

    return df_out_boxes
'''DO THE PLOTS TO CHECK'''
#boxes_L0 = getBoxes(df_5m_to_60m, threshhold)
print('done getting boxes, finding parameters of the unbounded model')
...
def assignWvals(df_rain_upper_level,
    ...
    global dict_out_L1, dict_out_L2, result_simulations
...
#    np.random.seed(19680801) #check again
...
while simulation_nbr <= 10 : #upper limit to define nbr of simulations
    simulation_nbr += 1

    evaluation_L1 = assignWvals(df_5m_to_60m,
                            df_prob_p01_L1,
                            df_beta_params_L1,
...
lorenz_L1 = buildLorenzCurve_Original(df_5m_to_30m, cascade_level_1)
lorenz_L2 = buildLorenzCurve_Original(df_5m_to_15m, cascade_level_2)
