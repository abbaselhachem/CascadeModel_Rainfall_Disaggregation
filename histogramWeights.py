...
in_df_30min_orig = os.path.join(main_dir, r'resampled 30min.csv')
...
in_df_15min_orig = os.path.join(main_dir, r'resampled 15min.csv')
...
#get files for l1 and l2
dfs_files_L1 = []
for r, dir_, f in os.walk(os.path.join(in_data_dir, r'%s' % cascade_level_1)):
    for fs in f:
        if fs.endswith('.csv'):
            dfs_files_L1.append(os.path.join(r, fs))

dfs_files_L2 = []
for r, dir_, f in os.walk(os.path.join(in_data_dir, r'%s' % cascade_level_2)):
    for fs in f:
        if fs.endswith('.csv'):
            dfs_files_L2.append(os.path.join(r, fs))
            ...
            if station in df_file:
                    ...
                    for i, col in enumerate(d.columns):
                        ...
                        if i == 0:
                            ...
                            ax.set_ylabel('Normed Weights Count', fontsize=14)
                            ...
                            plt.title((r'%s cascade %s'
                                       '\nnumber of $0<W<1$: %d')\
                                        %(station, cascade_level,
                                          len(d[col].index)), fontsize=16)

                            plt.text(0.86, 1.01, (r'$\beta$=%0.2f' %a),
                                     withdash=True, fontsize=16)
...
def plotProbMonth(prob_df_file, cascade_level):
    ...
    ax2.yaxis.set_ticks(np.arange(0, 0.36, 0.025))

    plt.title(r'Seasonal Effects on $P_{01}$ for Cascade %s'\
              % cascade_level, fontsize=18)

    ax2.set_xlabel(r'Month', fontsize=16)
    ax2.set_ylabel('P($W_{1}$)', fontsize=16)

    legend = ax2.legend(loc=2, prop={'size':13})
    frame = legend.get_frame()
    frame.set_facecolor('0.9')
    frame.set_edgecolor('0.9')
...
#    ax3.set(xticks=[])
    ax3.yaxis.set_ticks(np.arange(0, max(y_3), 0.025))
    ...
    plt.title(('$P_{01}$ per Station'
              ' Cascade %s') %cascade_level, fontsize=18)

    ax3.set_ylabel('P(W1)', fontsize=18)
    ax3.set_xlabel('Station ID', fontsize=18)
...
dfs_files_P01_L1 = []
for r, dir_, f in os.walk(os.path.join(unbounded_model_dir_L1)):
    for fs in f:
        if fs.endswith('.csv'):
            dfs_files_P01_L1.append(os.path.join(r, fs))

#read df values of R_vals, W_vals and logLikelihood vals L2
dfs_files_P01_L2 = []
for r, dir_, f in os.walk(os.path.join(unbounded_model_dir_L2)):
    for fs in f:
        if fs.endswith('.csv'):
            dfs_files_P01_L2.append(os.path.join(r, fs))

def volumeDependacyP01(in_df_files, in_param_file, cascade_level):

    percentile = 0.03 #to divide R values into classes and fill classes with W
        ...
        for stn in stn_list:
            ...
            if fnmatch.fnmatch(df_file,'*.csv') and stn in df_file:
                ...
                if len(d['R vals'].values) >= min_w_nbrs:
                    ...
                    ax.set_xlabel('log$_{10}$ R', fontsize = 16)
                    ax.set_ylabel('P$_{01}$', fontsize = 16)
                    ax.grid(color='k', linestyle='-', linewidth=0.1)

                    plt.title('%s Volume dependence of P$_{01}$ for Cascade %s'\
                              %(stn,cascade_level), fontsize = 18 )

                    legend = ax.legend(loc=1, prop={'size':13})
                    frame = legend.get_frame()
                    frame.set_facecolor('0.9')
                    frame.set_edgecolor('0.9')
...
#call fct
#compareHistRain(in_df_30min_orig, dfs_files_sim, cascade_level_1)
#compareHistRain(in_df_15min_orig, dfs_files_sim_2, cascade_level_2)
...
#                        print(x_axis_0)
                        ...
                        plt.scatter(x_axis_2,
                            ...
                            marker='*', c = 'm',
                            alpha=0.75,
                            label='dependent extremes %s' %len\
                            (df_extremes_unbound['unbounded extremes'].values))
...
#        except ContinueI:
#            continue
compareExtremes(in_df_30min_orig, dfs_files_sim, cascade_level_1)
compareExtremes(in_df_15min_orig, dfs_files_sim_2, cascade_level_2)
raise Exception
#==============================================================================
#
#==============================================================================
...
#        try:
                ...
                plt.title\
                ('%s Bounds of Simulations Cascade %s' %(station,
                                                         cascade_level),
                fontsize=18)
...
####recheck why values of em3 and em4, em10, em11 and em13, em14 are the same!!!
